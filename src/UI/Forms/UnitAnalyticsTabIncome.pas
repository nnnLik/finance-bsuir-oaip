...
type
  TAnalyticsIncomeTab = class(TPanel)
  private
    FPaint: TPaintBox;
    FEmpty: TLabel;
...
constructor TAnalyticsIncomeTab.Create(AOwner: TComponent);
begin
...
procedure TAnalyticsIncomeTab.RefreshData(const Scope: string;
  const ActiveAccountId: Int64);
var
  D: TCategoryChartPointArray;
begin
  BuildIncomeCategories(Scope, ActiveAccountId, D);
  FData := D;
...
procedure TAnalyticsIncomeTab.PaintChart(Sender: TObject);
var
  C: TCanvas;
  R: TRect;
